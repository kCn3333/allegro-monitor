...
async function render() {
  ...
  $("#watched").innerHTML = data.watched.map(item => {
    const clientEnabled = item.clientEnabled !== false;
    const status = !clientEnabled ? "Wyłączone na tym urządzeniu" : item.tabOpen === false ? "Karta zamknięta" : "Karta otwarta";
    const statusClass = clientEnabled && item.tabOpen !== false ? "open" : "closed";
    const primary = !clientEnabled
      ? `<button data-activate="${item.monitorId}">Włącz obserwowanie</button>`
      : item.tabOpen === false
        ? `<button data-open="${item.monitorId}">Otwórz kartę</button>`
        : `<button data-check="${item.monitorId}">Sprawdź teraz</button>`;
    const deactivate = clientEnabled ? `<button class="deactivate" data-deactivate="${item.monitorId}">Wyłącz tutaj</button>` : "";
    return `<div class="watch"><div class="watch-head"><strong>${esc(item.name)}</strong>${Number(item.lastCheckNewCount) > 0 ? `<span class="new-count" title="Nowe w ostatnim sprawdzeniu">+${Number(item.lastCheckNewCount)}</span>` : ""}</div><span class="tab-status ${statusClass}">${status}</span><span> · ${Number(item.activeClientsCount) || 0} aktywnych urządzeń · co ${item.intervalMinutes} min</span><div class="actions">${primary}${deactivate}</div></div>`;
  }).join("");
...
document.addEventListener("click", async event => {
  ...
  const deactivate = event.target.dataset?.deactivate;
  const check = event.target.dataset?.check;
  if (open || activate) {
    const monitorId = Number(open || activate);
    const response = await send({ type:"client-state", monitorId, enabled:true });
    if (!response.ok) { message(response.error, "error"); return; }
  ...
  if (deactivate) {
    const response = await send({ type:"client-state", monitorId:Number(deactivate), enabled:false });
    message(response.ok ? "Wyłączono na tym urządzeniu" : response.error, response.ok ? "ok" : "error");
    await render();
  }
