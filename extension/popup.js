...
async function render() {
  ...
  $("#watched").innerHTML = data.watched.map(item => `<div class="watch"><div class="watch-head"><strong>${esc(item.name)}</strong>${Number(item.lastCheckNewCount) > 0 ? `<span class="new-count" title="Nowe w ostatnim sprawdzeniu">+${Number(item.lastCheckNewCount)}</span>` : ""}</div><span class="tab-status ${item.tabOpen === false ? "closed" : "open"}">${item.tabOpen === false ? "Karta zamknięta" : "Karta otwarta"}</span><span> · ${Number(item.activeClientsCount) || 0} aktywnych urządzeń · co ${item.intervalMinutes} min</span><div class="actions"><button data-open="${item.monitorId}">${item.tabOpen === false ? "Otwórz kartę" : "Pokaż kartę"}</button><button data-check="${item.monitorId}">Sprawdź teraz</button><button class="remove" data-remove="${item.monitorId}">Usuń</button></div></div>`).join("");
...
$("#sync").addEventListener("click", async () => { message("Synchronizowanie…"); const response=await send({type:"sync"}); message(response.ok ? "Lista zsynchronizowana" : response.error,response.ok ? "ok" : "error"); await render(); });
